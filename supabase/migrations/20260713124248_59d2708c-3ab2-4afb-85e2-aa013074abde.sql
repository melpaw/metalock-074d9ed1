...
-- 1) admin_add_transaction: pending deposit locks funds so user sees it in balance
CREATE OR REPLACE FUNCTION public.admin_add_transaction(_user_id uuid, _type tx_type, _currency_id uuid, _amount numeric, _status text, _tx_hash text, _sender_address text, _note text, _hidden boolean, _tx_date timestamp with time zone, _fee_waived boolean DEFAULT false)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  VALUES (
    _user_id, _currency_id, _type, enum_status, signed, tx_fee, usd, _note,
    COALESCE(_hidden,false), auth.uid(), COALESCE(_tx_date, now()), _tx_hash,
    jsonb_build_object(
      'tx_hash', _tx_hash,
  ...
  IF _type = 'deposit' THEN
    ...
    ELSIF enum_status = 'pending' THEN
      INSERT INTO public.wallets (user_id, currency_id, available, locked)
      VALUES (_user_id, _currency_id, 0, net_amount)
      ON CONFLICT (user_id, currency_id) DO UPDATE
        SET locked = public.wallets.locked + net_amount;
    END IF;
  ELSIF enum_status = 'completed' THEN
    IF _type = 'withdrawal' THEN
      INSERT INTO public.wallets (user_id, currency_id, available)
...
-- 2) admin_update_transaction: transition deposit funds between locked/available
...
BEGIN
  ...
  IF old_status IS DISTINCT FROM enum_status THEN
    IF tx.type = 'deposit' THEN
      -- pending -> completed: locked -> available
      ...
      -- pending -> rejected/cancelled: remove from locked
      ...
      -- completed -> pending: available -> locked
      ...
      -- completed -> rejected/cancelled: remove from available
      ...
      -- rejected/cancelled -> pending: add back to locked
      ...
      -- rejected/cancelled -> completed: add to available
...
-- 3) Backfill: existing pending deposits should already reflect in locked
DO $$
DECLARE r RECORD; net numeric;
BEGIN
  FOR r IN SELECT * FROM public.transactions WHERE type='deposit' AND status='pending' LOOP
    net := GREATEST(abs(r.amount) - COALESCE(r.fee,0), 0);
    INSERT INTO public.wallets (user_id, currency_id, available, locked)
    VALUES (r.user_id, r.currency_id, 0, net)
    ON CONFLICT (user_id, currency_id) DO UPDATE
      SET locked = public.wallets.locked + net;
  END LOOP;
END $$;
