    ...
    <div className="space-y-4">
      <div className="rounded-sm border border-border bg-surface p-6 flex items-center justify-between">
        <div>
          ...
          <div className="text-lg font-semibold">{t(`profile.kyc.statuses.${status}`)}</div>
      ...
      {canSubmit && (
        <form onSubmit={submit} className="rounded-sm border border-border bg-surface p-6 grid gap-4 md:grid-cols-2">
          ...
          <div className="space-y-1.5"><Label>{t("profile.info.dob")}</Label><Input type="date" value={form.birth_date} onChange={(e) => setForm({ ...form, birth_date: e.target.value })} required /></div>
          <div className="space-y-1.5"><Label>{t("profile.info.country")}</Label><Input value={form.country} onChange={(e) => setForm({ ...form, country: e.target.value })} required /></div>
          <div className="space-y-1.5"><Label>{t("profile.kyc.docType")}</Label><Input value={form.doc_type} onChange={(e) => setForm({ ...form, doc_type: e.target.value })} required /></div>
          <div className="space-y-1.5"><Label>{t("profile.kyc.docNumber")}</Label><Input value={form.doc_number} onChange={(e) => setForm({ ...form, doc_number: e.target.value })} required /></div>
          <div className="md:col-span-2 space-y-1.5"><Label>{t("profile.info.address")}</Label><Input value={form.address} onChange={(e) => setForm({ ...form, address: e.target.value })} required /></div>
          <div className="space-y-1.5"><Label>{t("profile.kyc.documentPhoto")}</Label><Input type="file" accept="image/*,application/pdf" onChange={(e) => setDocFile(e.target.files?.[0] ?? null)} required /></div>
          <div className="space-y-1.5"><Label>{t("profile.kyc.bankStatement")}</Label><Input type="file" accept="image/*,application/pdf" onChange={(e) => setStatementFile(e.target.files?.[0] ?? null)} required /></div>
